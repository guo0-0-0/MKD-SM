...
from train_models.other_Models import VDSR_2channels
...
if __name__ == '__main__':
	...
	parser.add_argument('--warmup_epoch', type=int, default=110)
	...
	lr = 1e-2
	model = VDSR_2channels()
	model = model.to(device)
	model = torch.nn.DataParallel(model)
	optim = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=1e-4, nesterov=False)
	scheduler = lr_scheduler.StepLR(optim, step_size=total_epochs // 5, gamma=0.1)
	...
	model_path = root_path + 'result/down' + str(down) + '/VDSR.pth'
